...
extern "C" void HAL_CAN_RxFifo0MsgPendingCallback (CAN_HandleTypeDef *hcan)
{
        ...
        if (HAL_CAN_GetRxMessage (hcan, CAN_RX_FIFO0, &Can::can->rxHeader, rxData.data ()) != HAL_OK) {
                /* Reception Error */
                Error_Handler ();
        }
...
extern "C" void CAN1_TX_IRQHandler (void) { HAL_CAN_IRQHandler (&Can::can->canHandle); }
...
{
        uint32_t txMailbox{};
        return HAL_CAN_AddTxMessage (&canHandle, &txHeader, const_cast<uint8_t *> (&buff.data[0]), &txMailbox) == HAL_OK;
